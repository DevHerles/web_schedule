...
function get_fc_defaultOptions() {
    ...
    // NB: should be removed when fullschedule is updated to 2.0 because it would
    // be supported. See the following link
    // http://fullschedule.io/wiki/Upgrading-to-v2/
    var timeFormat = time.strftime_to_moment_format(_t.database.parameters.time_format).replace('A', 'TT');

    // adapt format for fullschedule v1.
    // see http://fullschedule.io/docs1/utilities/formatDate/
    ...
    return {
        ...
        // http://fullschedule.io/docs1/text/timeFormat/
...
var ScheduleView = View.extend({
    ...
    init: function () {
        ...
        // If this field is set to true, we will use the schedule_friends model as filter and not the color field.
        ...
        */
        ...
        if (this.color_field && this.selected_filters.length === 0) {
            var default_filter;
            if ((default_filter = this.dataset.context['schedule_default_' + this.color_field])) {
    ...
    start: function () {
        this.$schedule = this.$(".o_calendar_widget_");
    ...
    destroy: function() {
        ...
        if (this.$small_schedule) {
            this.$small_schedule.datepicker('destroy');
        }
        this._super.apply(this, arguments);
    },
    _do_show_init: function () {
        this.init_schedule().then(function() {
            $(window).trigger('resize');
        });
    },
    /**
     * Render the buttons according to the ScheduleView.buttons template and
     * add listeners on it.
    ...
    toggle_full_width: function () {
        var full_width = (local_storage.getItem('web_schedule_full_width') !== 'true');
        local_storage.setItem('web_schedule_full_width', full_width);
        this.toggle_sidebar(!full_width);
        this.$schedule.fullCalendar('render'); // to reposition the events
    },
    get_fc_init_options: function () {
        //Documentation here : http://arshaw.com/fullschedule/docs/
        var self = this;
        return $.extend({}, get_fc_defaultOptions(), {
            ...
            viewRender: function(view) {
                ...
                self.$schedule.fullCalendar('option', 'height', Math.max(290, parseInt(self.$('.o_calendar_view').height())));

                setTimeout(function() {
                    var $fc_view = self.$schedule.find('.fc-view');
                    var width = $fc_view.find('> table').width();
                    $fc_view.find('> div').css('width', (width > $fc_view.width())? width : '100%'); // 100% = fullCalendar default
                }, 0);
            },
            windowResize: function() {
                self.$schedule.fullCalendar('render');
            },
            ...
            select: function (start_date, end_date, all_day, _js_event, _view) {
                ...
                //self.set_employee_avatar(data_template);
            },

            unselectAuto: false,
        });
    },
    scheduleMiniChanged: function (context) {
        return function(datum,obj) {
            var curView = context.$schedule.fullCalendar('getView');
            ...
            if (curView.name == "agendaWeek") {
                if (curDate <= curView.end && curDate >= curView.start) {
                    context.$schedule.fullCalendar('changeView','agendaDay');
                }
            }
            else if (curView.name != "agendaDay" || (curView.name == "agendaDay" && moment(curDate).diff(moment(curView.start))===0)) {
                context.$schedule.fullCalendar('changeView','agendaWeek');
            }
            context.$schedule.fullCalendar('gotoDate', obj.currentYear , obj.currentMonth, obj.currentDay);
        };
    },
    init_schedule: function() {
        ...
        if (!this.sidebar) {
            ...
            this.$small_schedule = this.$(".o_calendar_mini");
            this.$small_schedule.datepicker({ 
                onSelect: this.scheduleMiniChanged(this),
    ...
    get_employee_avatar_class: function () {
        return widgets.EmployeeAvatar;
    },
    set_employee_avatar: function(data_template) {
        alert('set_employee_avatar');
    },
        ...
        self.promises.push(
            self.ScheduleModel.call('load_schedule_data', [
                ...
                if (response != undefined) {
                    self.employess = response.employees;
    ...
    load_employees: function() {
        var self = this;
        self.promises.push(
            self.ScheduleModel.call('load_employees', [
                {
                    schedule_id: self.schedule_id,
                }
            ]).then(function (response) {
                if (response != undefined) {
                    self.employess = response.employees;
                    self.attendee_people = response.employees;
                }else {
                    self.employees = [];
                }
            })
    ...
    open_quick_create: function(data_template) { // FIXME
        ...
        this.quick.on('added', this, this.quick_created)
                ...
                .on('closed', this, function() {
                    delete this.quick;
                    this.$schedule.fullCalendar('unselect');
                });
    ...
    /**
     * Refresh one fullschedule event identified by it's 'id' by reading OpenERP record state.
     * If event was not existent in fullschedule, it'll be created.
     */
    refresh_event: function(id) {
        ...
        this.dataset.read_ids([id], _.keys(this.fields)).done(function (incomplete_records) {
            self.perform_necessary_name_gets(incomplete_records).then(function(records) {
                // Event boundaries were already changed by fullschedule, but we need to reload them:
                var new_event = self.event_data_transform(records[0]);
                // fetch event_obj
                var event_objs = self.$schedule.fullCalendar('clientEvents', id);
                if (event_objs.length == 1) { // Already existing obj to update
                    ...
                    self.$schedule.fullCalendar('updateEvent', event_obj);
                } else { // New event object to create
                    var $fc_view = self.$schedule.find('.fc-view');
                    var scrollPosition = $fc_view.scrollLeft();
                    $fc_view.scrollLeft(0);
                    self.$schedule.fullCalendar('renderEvent', new_event);
                    ...
                    self.$schedule.fullCalendar('clientEvents', id)[0].source = self.event_source;
                }
            });
        });
    },
    ...
    /**
     * Transform OpenERP event object to fullschedule event object
     */
    ...
    /**
     * Transform fullschedule event object to OpenERP Data object
     */
    get_event_data: function(event) {
        ...
        // Normalize event_end without changing fullschedules event.
        var data = {
            name: event.title
        };            
        ...
        if (event.allDay) {
            // Sometimes fullschedule doesn't give any event.end.
    ...
    _do_search: function(domain, context, _group_by) {
        ...
        if (! _.isUndefined(this.event_source)) {
            this.$schedule.fullCalendar('removeEventSource', this.event_source);
        }
        this.event_source = {
        ...
        this.$schedule.fullCalendar('addEventSource', this.event_source);
    },
    /**
        ...
        if (index !== null) {
            ...
            this.dataset.write(event_id, data, {context: {from_ui: true}}).always(function() {
                if (is_virtual_id(event_id)) {
                    ...
                    self.$schedule.fullCalendar('refetchEvents');
                } else {
    ...
    remove_event: function(id) {
        ...
        function do_it() {
            return $.when(self.dataset.unlink([id])).then(function() {
                self.$schedule.fullCalendar('removeEvents', id);
            });
        }
